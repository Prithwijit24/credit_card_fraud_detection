...
@patch("fraud_detection.jobs.producer.Path.open", new_callable=mock_open, read_data=""",trans_date_trans_time,cc_num,merchant,category,amt,first,last,gender,street,city,state,zip,lat,long,city_pop,job,dob,trans_num,unix_time,merch_lat,merch_long,is_fraud
0,2020-01-01 00:00:00,123,abc,shopping,100.0,John,Doe,M,123 Main St,Anytown,CA,12345,34.0,-118.0,1000,Teacher,1980-01-01,txn1,1577836800,34.1,-118.1,0
""")
def test_producer_main(mock_file, mock_load_config, mock_parse_args, mock_kafka_producer, project_root):
    mock_args = MagicMock()
    ...
    mock_config.streaming = {"kafka_topic": "fraud_events", "kafka_bootstrap_servers": "localhost:9092"}
